...
def preprocessing_fn(inputs):
    ...
    for key in _VOCAB_FEATURE_KEYS:
        outputs[_transformed_name(key)] = tft.compute_and_apply_vocabulary(x=inputs[key],
                                                                           top_k=_VOCAB_SIZE,
    ...
    for key in _BUCKET_FEATURE_KEYS:
        outputs[_transformed_name(key)] = tft.bucketize(x=inputs[key],
                                                        num_buckets=_FEATURE_BUCKET_COUNT.get(key)
                                                       )
    ...
    traffic_volume = tf.cast(x=inputs[_VOLUME_KEY], dtype=tf.float32)
